    ...
    try {
      ...
      if (!splitPath.toFile.exists()) {
        throw new RuntimeException(
          s"Split file was not created at: $splitPath (convertIndexFromPath returned: $splitBuildRc)"
        )
      }
      ...
      try {
        ...
        for ((hit, i) <- hits.zipWithIndex) {
          ...
          if (document != null) {
            ...
            println(s"Published: $publishedValue (${if (publishedValue != null) publishedValue.getClass.getSimpleName
              else "null"})")
            ...
            assert(
              titleValue.toString.contains("Debug Document"),
              "Title should contain expected text, contained" + titleValue
            )
