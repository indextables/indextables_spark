...
class V2MultiPathTest extends TestBase with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  ignore("should read from multiple individual paths") {
    withTempPath { basePath =>
      ...
      data1.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(path1)
      data2.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(path2)
      data3.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(path3)

      // Read from individual paths
      val result1 = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(path1)
      val result2 = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(path2)
      val result3 = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(path3)
  ...
  ignore("should handle mixed data types across multiple paths") {
    withTempPath { basePath =>
      ...
      numericData.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(numericPath)
      textData.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(textPath)
  ...
  ignore("should handle overlapping path hierarchies") {
    withTempPath { basePath =>
      ...
      parentData.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(parentPath)
      childData.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(childPath)
      siblingData.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(siblingPath)

      // Read each path individually to verify separation
      val parentResult  = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(parentPath)
      val childResult   = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(childPath)
      val siblingResult = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(siblingPath)
  ...
  ignore("should handle empty paths in multi-path operations") {
    withTempPath { basePath =>
      ...
      data.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(dataPath)
      ...
      emptyData.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(emptyPath)
  ...
  ignore("should handle path resolution edge cases") {
    withTempPath { basePath =>
      ...
      data.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(normalPath)
      data.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(pathWithSpaces)
      data.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(pathWithSpecialChars)

      // Read from each path
      val normalResult = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(normalPath)
      val spacesResult = spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(pathWithSpaces)
      val specialResult =
        spark.read.format("io.indextables.spark.core.Tantivy4SparkTableProvider").load(pathWithSpecialChars)
  ...
  ignore("should support different file protocols in theory") {
    ...
    withTempPath { basePath =>
      ...
      data.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(localPath)
  ...
  ignore("should handle large number of paths efficiently") {
    withTempPath { basePath =>
      ...
      val paths = (0 until numPaths).map { i =>
        ...
        data.write.format("io.indextables.spark.core.Tantivy4SparkTableProvider").mode("overwrite").save(path)
        path
      }
