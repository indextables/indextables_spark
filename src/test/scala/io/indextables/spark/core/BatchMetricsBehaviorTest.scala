 ...
 * Each query should show correct per-query metrics via getMetricsDelta().
 */
class BatchMetricsBehaviorTest extends TestBase {
  ...
  test("per-query metrics work correctly with baseline/delta approach") {
    ...
    if (result1.length >= 50) {
      assert(delta1.totalOperations >= 1,
        s"Expected at least 1 batch op for ${result1.length} docs, got ${delta1.totalOperations}")
      assert(delta1.totalDocuments > 0,
        s"Expected totalDocuments > 0 for ${result1.length} rows")
      assert(delta1.consolidatedRequests < delta1.totalRequests || delta1.totalRequests == 0,
      ...
      assert(delta1.totalOperations == 0,
        s"Expected 0 batch ops below 50-doc threshold, got ${delta1.totalOperations}")
  ...
  test("limit(100) x5 - each query should show consistent per-query metrics") {
    ...
    for (i <- 1 to 5) {
      ...
      assert(delta.consolidatedRequests > 0 && delta.consolidatedRequests <= 100,
  ...
  test("limit(500) x3 - each query should show consistent per-query metrics") {
    ...
    for (i <- 1 to 3) {
      ...
      assert(delta.consolidatedRequests > 0 && delta.consolidatedRequests <= 500,
  ...
  test("docBatchMaxSize=5000 with 10000 docs should result in exactly 2 batch operations") {
    ...
    assert(result.length == 10000,
      s"Expected 10000 rows, got ${result.length}")

    // Validate exactly 2 batch operations (10000 docs / 5000 per batch = 2)
    assert(delta.totalOperations == 2,
      s"Expected exactly 2 batch ops (10000/5000), got ${delta.totalOperations}")

    // Validate total docs matches rows returned
    assert(delta.totalDocuments == 10000,
      s"Expected 10000 docs in metrics, got ${delta.totalDocuments}")

    // Validate total requests equals docs (one request per doc before consolidation)
    assert(delta.totalRequests == 10000,
      s"Expected 10000 baseline requests, got ${delta.totalRequests}")
