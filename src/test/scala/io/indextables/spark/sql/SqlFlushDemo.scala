...
object SqlFlushDemo extends App {
  ...
  try {
    ...
    try {
      ...
      testData.write.format(io.indextables.spark.TestBase.INDEXTABLES_FORMAT).mode("overwrite").save(tempPath)

      // Read it back to create some cache entries
      val readData = spark.read.format(io.indextables.spark.TestBase.INDEXTABLES_FORMAT).load(tempPath)
