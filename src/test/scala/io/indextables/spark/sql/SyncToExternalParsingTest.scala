...
/**
 * Tests for SYNC INDEXTABLES TO DELTA SQL parsing.
 *
...
class SyncToExternalParsingTest extends TestBase {
  ...
  test("parse basic SYNC INDEXTABLES TO DELTA syntax") {
    val cmd = parseSync(
  ...
  test("parse SYNC with TANTIVY4SPARK keyword") {
    val cmd = parseSync(
      "SYNC TANTIVY4SPARK TO DELTA 's3://bucket/delta_table' AT LOCATION 's3://bucket/index'"
    )
    cmd.sourceFormat shouldBe "delta"
    cmd.sourcePath shouldBe "s3://bucket/delta_table"
    cmd.destPath shouldBe "s3://bucket/index"
  ...
  test("parse SYNC with DRY RUN") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA 's3://data/events' AT LOCATION 's3://idx/events' DRY RUN"
    )
  ...
  test("parse SYNC with FASTFIELDS MODE HYBRID") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/tmp/delta' FASTFIELDS MODE HYBRID AT LOCATION '/tmp/index'"
    )
    cmd.fastFieldMode shouldBe "HYBRID"
  }
  ...
  test("parse SYNC with FASTFIELDS MODE DISABLED") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/tmp/delta' FASTFIELDS MODE DISABLED AT LOCATION '/tmp/index'"
    )
  ...
  test("parse SYNC with FASTFIELDS MODE PARQUET_ONLY") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/tmp/delta' FASTFIELDS MODE PARQUET_ONLY AT LOCATION '/tmp/index'"
    )
  ...
  test("parse SYNC with TARGET INPUT SIZE") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/tmp/delta' TARGET INPUT SIZE 2G AT LOCATION '/tmp/index'"
    )
  ...
  test("parse SYNC with TARGET INPUT SIZE in megabytes") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/tmp/delta' TARGET INPUT SIZE 500M AT LOCATION '/tmp/index'"
    )
    cmd.targetInputSize shouldBe defined
  ...
  test("parse SYNC with INDEXING MODES") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/tmp/delta' INDEXING MODES ('title':'text', 'ip_addr':'ipaddress') AT LOCATION '/tmp/index'"
    )
  ...
  test("parse SYNC with all options combined") {
    val cmd = parseSync(
      """SYNC INDEXTABLES TO DELTA 's3://bucket/delta'
  ...
  test("parse SYNC with local filesystem paths") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/data/my_delta_table' AT LOCATION '/data/my_index'"
    )
  ...
  test("parse SYNC with Azure paths") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA 'abfss://container@account.dfs.core.windows.net/delta' AT LOCATION 'abfss://container@account.dfs.core.windows.net/index'"
    )
  ...
  test("default fastFieldMode should be HYBRID") {
    val cmd = parseSync(
      "SYNC INDEXTABLES TO DELTA '/tmp/delta' AT LOCATION '/tmp/index'"
    )
    cmd.fastFieldMode shouldBe "HYBRID"
  }
