...
class IndexTables4SparkFullIntegrationTest extends TestBase {
  ...
  test("should perform full write/read cycle with comprehensive test data") {
    ...
    withTempPath { tempPath =>
      ...
      filteredData.foreach(row => row.getAs[Int]("age") should be > 30)
    }
  }
  ...
  test("should handle equality and IN queries") {
    ...
    withTempPath { tempPath =>
      ...
      engineeringResults.foreach(row => row.getAs[String]("department") shouldBe "Engineering")
      ...
      activeResults.foreach(row => row.getAs[String]("status") shouldBe "active")
      ...
      usaResults.foreach(row => row.getAs[String]("country") shouldBe "USA")
      ...
      deptInResults.foreach(row => validDepts should contain(row.getAs[String]("department")))
      ...
      statusInResults.foreach(row => validStatuses should contain(row.getAs[String]("status")))
      ...
      priorityInResults.foreach(row => validPriorities should contain(row.getAs[Int]("priority")))
      ...
      deptNotInResults.foreach(row => excludedDepts should not contain (row.getAs[String]("department")))
      ...
      statusNotInResults.foreach(row => excludedStatuses should not contain (row.getAs[String]("status")))
  ...
  test("should handle boolean queries") {
    ...
    withTempPath { tempPath =>
      ...
      activeTrueResults.foreach(row => row.getAs[Boolean]("is_active") shouldBe true)
      ...
      verifiedFalseResults.foreach(row => row.getAs[Boolean]("is_verified") shouldBe false)
      ...
      premiumTrueResults.foreach(row => row.getAs[Boolean]("has_premium") shouldBe true)
  ...
  test("should handle date and timestamp queries") {
    withTempPath { tempPath =>
      ...
      dateQueries.foreach(query => noException should be thrownBy query.collect().length)
    }
  }
  ...
  test("should handle complex compound queries") {
    withTempPath { tempPath =>
      ...
      complexQueries.foreach(query => noException should be thrownBy query.collect().length)
    }
  }
  ...
  test("should handle aggregation queries with filters") {
    withTempPath { tempPath =>
      ...
      aggregationQueries.foreach(query => noException should be thrownBy query.collect().length)
    }
  }
  ...
  test("should complete full write/read/query cycle with comprehensive filters") {
    ...
    withTempPath { tempPath =>
      ...
      mlBioResults.foreach(row => row.getAs[String]("bio").toLowerCase should include("machine learning"))

      // Test: Text search - title starts with "Senior"
      val seniorTitleResults = readData.filter(col("title").startsWith("Senior")).collect()
      seniorTitleResults.foreach(row => row.getAs[String]("title") should startWith("Senior"))
      ...
      sfLocationResults.foreach(row => row.getAs[String]("location") should endWith("Francisco"))
      ...
      highSalaryResults.foreach(row => row.getAs[Double]("salary") should be > 70000.0)
  ...
  private def createTextSearchDataFrame(): DataFrame = {
    ...
    val textData = Seq(
      (
        ...
        "Learn Apache Spark with this comprehensive guide",
        "Learn about distributed computing with Apache Spark engine",
        "technology",
        "spark,tutorial,big-data"
      ),
      (
        2,
        "Machine Learning Basics",
        "Introduction to ML algorithms",
        "Explore machine learning fundamentals and algorithms",
        "AI",
        "ML,algorithms,AI"
      ),
      (
        ...
        "Master data analysis and statistical methods",
        "data",
      ...
      (
        ...
        "Functional programming with Scala language",
        "programming",
