...
/**
 * Real AWS S3 integration tests for prescan filtering and warmup.
 *
 * Tests prescan functionality against real S3 storage:
 ...
 *   - Error handling for S3-specific scenarios
 *
 * Credentials are loaded from ~/.aws/credentials file.
 * To run: mvn test-compile scalatest:test -DwildcardSuites='io.indextables.spark.prescan.RealS3PrescanIntegrationTest'
 */
class RealS3PrescanIntegrationTest extends RealS3TestBase {
  ...
  private val testBasePath = s"$S3_BASE_PATH/prescan-test-$testRunId"
  ...
  /** Get read options with AWS credentials and prescan enabled. */
  ...
  private def cleanupTestData(): Unit = {
    println(s"📦 Test data at $testBasePath - cleanup skipped (unique paths used)")
  }

  // ==================== PRESCAN FILTERING TESTS ====================

  test("prescan should eliminate splits on S3 for non-matching equality filter") {
    assume(awsCredentials.isDefined, "AWS credentials required for this test")

    val tablePath = s"$testBasePath/equality-filter-${generateTestId()}"
    ...
    println("✅ Prescan equality filter test passed on S3")
  }

  test("prescan should work with aggregates and filter on S3") {
    assume(awsCredentials.isDefined, "AWS credentials required for this test")

    val tablePath = s"$testBasePath/aggregate-${generateTestId()}"
    ...
    println("✅ Prescan aggregate test passed on S3")
  }

  // ==================== PREWARM COMMAND TESTS ====================

  test("PREWARM PRESCAN FILTERS command should work on S3") {
    assume(awsCredentials.isDefined, "AWS credentials required for this test")

    val tablePath = s"$testBasePath/prewarm-${generateTestId()}"
    ...
    println("✅ PREWARM command test passed on S3")
  }

  // ==================== ENABLE/DISABLE COMMANDS ====================

  test("ENABLE/DISABLE PRESCAN FILTERING commands should work on S3") {
    assume(awsCredentials.isDefined, "AWS credentials required for this test")

    val tablePath = s"$testBasePath/enable-disable-${generateTestId()}"
    ...
    println("✅ ENABLE/DISABLE commands test passed on S3")
  }

  test("prescan should benefit from cached footers on S3") {
    assume(awsCredentials.isDefined, "AWS credentials required for this test")

    val tablePath = s"$testBasePath/cache-test-${generateTestId()}"
    ...
    println("✅ Prescan cache behavior test passed on S3")
  }
}
