...
/**
 * Real S3 integration tests for SYNC INDEXTABLES TO DELTA.
 *
 * These tests require:
...
class RealS3SyncToExternalTest extends RealS3TestBase {
  ...
  test("SYNC INDEXTABLES TO DELTA should parse with S3 paths") {
    val result = spark.sql(
      s"SYNC INDEXTABLES TO DELTA '${testBasePath}/delta_source' AT LOCATION '${testBasePath}/index_dest' DRY RUN"
    )
  ...
  test("SYNC command output schema should have 11 columns") {
    val result = spark.sql(
      s"SYNC INDEXTABLES TO DELTA '/nonexistent/delta' AT LOCATION '/nonexistent/index' DRY RUN"
    )
  ...
  test("SYNC command should return error status for non-existent Delta table") {
    val result = spark.sql(
      s"SYNC INDEXTABLES TO DELTA '/tmp/nonexistent_delta_${testRunId}' AT LOCATION '/tmp/nonexistent_index_${testRunId}'"
    )
