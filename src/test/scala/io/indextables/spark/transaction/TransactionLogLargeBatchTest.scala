...
/**
 * Integration test to validate transaction log file naming with large batches (>100 actions).
 * This test ensures the bug where version files were named "2_000.json" instead of
 * "00000000000000000002.json" is caught and prevented.
 */
  ...
  test("transaction log should create properly named files when writing >100 actions") {
    withTempPath { tempDir =>
      ...
      try {
        ...
        val wrongPatternFiles = versionFiles.filter(f =>
          f.matches("\\d+_\\d+\\.json") // Pattern like 1_000.json
        )
  ...
  test("transaction log should create properly named files for versions 2-24") {
    withTempPath { tempDir =>
      ...
      try {
        ...
        val fs = transactionLogPath.getFileSystem(spark.sparkContext.hadoopConfiguration)
        val files = fs.listStatus(transactionLogPath)
        val versionFiles = files.map(_.getPath.getName)
