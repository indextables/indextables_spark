 ...
 *
 * These tests verify that all SQL commands work correctly when the table uses the Avro state format for checkpoints.
 *
 * Run with: mvn scalatest:test -DwildcardSuites='io.indextables.spark.transaction.avro.AvroStateCommandsTest'
 */
class AvroStateCommandsTest extends TestBase {
  ...
  test("DROP PARTITIONS with range predicate works with Avro state") {
    withTempPath { tempDir =>
      ...
      val data = (1 to 10).map(i => (i, s"name_$i", f"2024-01-$i%02d", i * 10))
  ...
  test("DROP PARTITIONS with partition predicate should use Avro manifest pruning") {
    withTempPath { tempDir =>
      ...
      val dropResult2 =
        spark.sql(s"DROP INDEXTABLES PARTITIONS FROM '$path' WHERE region IN ('us-west', 'eu-west')").collect()
      ...
      val apEastCount = spark.read
        .format(provider)
        .load(path)
        .filter(col("region") === "ap-east")
        .count()
  ...
  test("MERGE SPLITS with WHERE clause should use Avro manifest pruning") {
    withTempPath { tempDir =>
      ...
      val euWestCount = spark.read
        .format(provider)
        .load(path)
        .filter(col("region") === "eu-west")
        .count()
  ...
  test("MERGE SPLITS with WHERE clause should ONLY merge targeted partition files") {
    withTempPath { tempDir =>
      ...
      val txLog =
        io.indextables.spark.transaction.TransactionLogFactory.create(new org.apache.hadoop.fs.Path(path), spark)
  ...
  test("PREWARM CACHE with WHERE clause should use Avro manifest pruning") {
    withTempPath { tempDir =>
      ...
      // Verify the status
      prewarmResult.foreach(row => Seq("success", "no_splits") should contain(row.getAs[String]("status")))
    }
  }
  ...
  test("Commands with compound predicates should use Avro manifest pruning") {
    withTempPath { tempDir =>
      ...
      val droppedPartitionCount = spark.read
        .format(provider)
        .load(path)
        .filter(col("year") === "2023" && col("month") === "06")
        .count()
      droppedPartitionCount shouldBe 0

      // Other 2023 partitions should still exist
      val year2023Count = spark.read
        .format(provider)
        .load(path)
  ...
  test("Multiple manifests scenario should benefit from partition pruning") {
    withTempPath { tempDir =>
      ...
      val regionCCount = spark.read
        .format(provider)
        .load(path)
        .filter(col("region") === "region-C")
        .count()
  ...
  test("Transaction log correctly tracks files across multiple Avro state versions") {
    withTempPath { tempDir =>
      ...
      val stateDirs1 = cloudProvider1
        .listFiles(txLogPath, recursive = false)
      ...
      assert(stateDirs2.size >= 2, "Expected at least 2 state directories after two writes")
