...
/**
 * Base class for cloud Azure Blob Storage integration tests.
 *
 ...
 */
abstract class CloudAzureTestBase extends AnyFunSuite with Matchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  override def beforeAll(): Unit = {
    ...
    val builder = SparkSession
      .builder()
      .appName("CloudAzureIntegrationTest")
      .master("local[2]")
    ...
    println(s"🚀 CloudAzureTestBase: Spark session created for real Azure Blob Storage testing")
  }

  override def afterAll(): Unit = {
    if (spark != null) {
      spark.stop()
      println(s"🛑 CloudAzureTestBase: Spark session stopped")
    }
    super.afterAll()
  }
