...
class AutoSizeOptionsUnitTest extends AnyFunSuite with Matchers {
  ...
  test("Tantivy4SparkOptions should parse autoSizeEnabled correctly") {
    // Test enabled
    val enabledOptions = Map("spark.tantivy4spark.autoSize.enabled" -> "true")
    ...
    // Test disabled
    val disabledOptions = Map("spark.tantivy4spark.autoSize.enabled" -> "false")
  ...
  test("Tantivy4SparkOptions should parse autoSizeTargetSplitSize correctly") {
    // Test with megabytes
    val mbOptions = Map("spark.tantivy4spark.autoSize.targetSplitSize" -> "100M")
    ...
    // Test with gigabytes
    val gbOptions = Map("spark.tantivy4spark.autoSize.targetSplitSize" -> "2G")
    ...
    // Test with bytes
    val bytesOptions = Map("spark.tantivy4spark.autoSize.targetSplitSize" -> "1048576")
  ...
  test("Tantivy4SparkOptions should parse autoSizeInputRowCount correctly") {
    ...
    val rowCountOptions = Map("spark.tantivy4spark.autoSize.inputRowCount" -> "10000")
    ...
    val largeCountOptions = Map("spark.tantivy4spark.autoSize.inputRowCount" -> "1000000")
  ...
  test("Tantivy4SparkOptions should handle invalid autoSizeInputRowCount gracefully") {
    ...
    val invalidOptions = Map("spark.tantivy4spark.autoSize.inputRowCount" -> "not_a_number")
  ...
  test("Tantivy4SparkOptions should combine all options correctly") {
    val fullOptions = Map(
      "spark.tantivy4spark.autoSize.enabled" -> "true",
      "spark.tantivy4spark.autoSize.targetSplitSize" -> "100M",
      "spark.tantivy4spark.autoSize.inputRowCount" -> "50000",
      "optimizeWrite" -> "true",
  ...
  test("Tantivy4SparkOptions constants should match implementation") {
    ...
    assert(Tantivy4SparkOptions.AUTO_SIZE_ENABLED == "spark.tantivy4spark.autoSize.enabled")
    assert(Tantivy4SparkOptions.AUTO_SIZE_TARGET_SPLIT_SIZE == "spark.tantivy4spark.autoSize.targetSplitSize")
    assert(Tantivy4SparkOptions.AUTO_SIZE_INPUT_ROW_COUNT == "spark.tantivy4spark.autoSize.inputRowCount")
  }

  test("Tantivy4SparkOptions should work with Map constructor") {
    val optionsMap = Map(
      "spark.tantivy4spark.autoSize.enabled" -> "true",
      "spark.tantivy4spark.autoSize.targetSplitSize" -> "200M"
    )
  ...
  test("Tantivy4SparkOptions getAllOptions should include auto-sizing options") {
    val options = Map(
      "spark.tantivy4spark.autoSize.enabled" -> "true",
      "spark.tantivy4spark.autoSize.targetSplitSize" -> "500M",
      "spark.tantivy4spark.autoSize.inputRowCount" -> "100000",
      "other.option" -> "value"
    )
    ...
    assert(allOptions.contains("spark.tantivy4spark.autoSize.enabled"))
    assert(allOptions.contains("spark.tantivy4spark.autoSize.targetSplitSize"))
    assert(allOptions.contains("spark.tantivy4spark.autoSize.inputRowCount"))
    assert(allOptions.contains("other.option"))

    assert(allOptions("spark.tantivy4spark.autoSize.enabled") == "true")
    assert(allOptions("spark.tantivy4spark.autoSize.targetSplitSize") == "500M")
    assert(allOptions("spark.tantivy4spark.autoSize.inputRowCount") == "100000")
  }

  test("Tantivy4SparkOptions should handle empty and null values gracefully") {
    // Test with empty string values
    val emptyOptions = Map(
      "spark.tantivy4spark.autoSize.enabled" -> "",
      "spark.tantivy4spark.autoSize.targetSplitSize" -> "",
      "spark.tantivy4spark.autoSize.inputRowCount" -> ""
    )
  ...
  test("Tantivy4SparkOptions should handle boolean parsing edge cases") {
    ...
    trueVariants.foreach { trueValue =>
      val options = Map("spark.tantivy4spark.autoSize.enabled" -> trueValue)
    ...
    falseVariants.foreach { falseValue =>
      val options = Map("spark.tantivy4spark.autoSize.enabled" -> falseValue)
      val map = new CaseInsensitiveStringMap(options.asJava)
      val tantivyOptions = Tantivy4SparkOptions(map)
  ...
  test("Tantivy4SparkOptions should handle numeric parsing edge cases") {
    // Test zero row count
    val zeroOptions = Map("spark.tantivy4spark.autoSize.inputRowCount" -> "0")
    ...
    val largeOptions = Map("spark.tantivy4spark.autoSize.inputRowCount" -> "9223372036854775807") // Long.MAX_VALUE
