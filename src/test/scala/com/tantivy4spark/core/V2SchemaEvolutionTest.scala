...
class V2SchemaEvolutionTest extends TestBase with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  test("should handle column additions in schema evolution") {
    withTempPath { path =>
      ...
      val evolvedData = spark.range(10, 20).select(
        col("id"),
        concat(lit("User "), col("id")).as("name"),
  ...
  test("should handle column reordering gracefully") {
    withTempPath { path =>
      ...
      val reorderedData = spark.range(5, 10).select(
  ...
  test("should handle data type compatibility") {
    withTempPath { path =>
      ...
      val longData = spark.range(5, 10).select(
        col("id"),
  ...
  test("should handle backward compatibility with missing columns") {
    withTempPath { path =>
      ...
      val partialData = spark.range(8, 15).select(
        col("id"),
        concat(lit("Item "), col("id")).as("name")
      )
  ...
  test("should handle string to numeric evolution") {
    withTempPath { path =>
      ...
      val numericData = spark.range(6, 12).select(
        col("id"),
  ...
  test("should handle nullable to non-nullable evolution") {
    withTempPath { path =>
      ...
      val nonNullableData = spark.range(5, 10).select(
        col("id"),
  ...
  test("should handle complex schema merging") {
    withTempPath { path =>
      ...
      val contactData = spark.range(4, 7).select(
        col("id"),
        concat(lit("User "), col("id")).as("username"),
        (col("id") + 25).cast(IntegerType).as("age"),
      ...
      val prefData = spark.range(7, 10).select(
        col("id"),
        concat(lit("User "), col("id")).as("username"),
        (col("id") + 25).cast(IntegerType).as("age"),
        concat(lit("user"), col("id"), lit("@email.com")).as("email"),
      ...
      result.schema.fieldNames should contain theSameElementsAs Array(
        "id", "username", "age", "email", "phone", "newsletter_subscribed", "account_type"
      )
