 ...
 *
 * During AQE stage-wise execution, Spark serializes and deserializes task information
 * including our AddAction objects that contain footer metadata. This test ensures
 * that critical metadata (footerStartOffset, footerEndOffset, docMappingJson) is
 * properly preserved through this process.
 */
class AQEMetadataSerializationTest extends TestBase with Matchers {
  ...
  test("should serialize and deserialize AddAction with footer metadata correctly") {
    withTempPath { tempPath =>
      ...
      addActions.zipWithIndex.foreach { case (originalAction, index) =>
  ...
  test("should handle footer metadata in broadcast variables during AQE") {
    withTempPath { tempPath =>
      // Create test data
      val mainData = spark.range(400).select(
        col("id"),
        (col("id") % 10).cast("string").as("join_key"),
        (col("id") * 2).as("main_value")
      )

      val broadcastData = spark.range(10).select(
  ...
  test("should maintain footer metadata integrity across AQE stage boundaries") {
    withTempPath { tempPath =>
      ...
      val complexData = spark.range(500).select(
        col("id"),
  ...
  test("should validate footer metadata after AQE optimization decisions") {
    withTempPath { tempPath =>
      ...
      val optimizationData = spark.range(350).select(
        col("id"),
