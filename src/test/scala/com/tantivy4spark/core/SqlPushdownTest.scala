...
class SqlPushdownTest extends TestBase {
  ...
  private def verifyFilterPushdown(queryName: String, plan: SparkPlan, expectedFilterCount: Int): Unit = {
  ...
  test("should demonstrate pushdown effectiveness") {
    withTempPath { tempPath =>
      ...
      val largeData = spark.range(1000).select(
        col("id"),
      ...
      selectiveRows.toSeq.foreach { row =>
        row.getString(row.fieldIndex("status")) shouldBe "special"
      }
  ...
  test("should verify comprehensive predicate pushdown with multiple filter types") {
    withTempPath { tempPath =>
      ...
      complexResults.toSeq.foreach { row =>
        row.getString(row.fieldIndex("category")) shouldBe "vegetable"
      }
  ...
  test("should demonstrate pushdown vs no-pushdown performance difference") {
    withTempPath { tempPath =>
      ...
      val largeData = spark.range(5000).select(
        col("id"),
