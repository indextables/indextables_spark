...
class Tantivy4SparkOptionsAutoSizeTest extends TestBase {
  ...
  test("Tantivy4SparkOptions should handle case insensitive option keys") {
    ...
    val mixedCaseOptions = Map(
      "spark.tantivy4spark.autosize.enabled" -> "true", // lowercase
      "spark.tantivy4spark.AUTOSIZE.TARGETSPLITSIZE" -> "50M", // uppercase
      "spark.tantivy4spark.AutoSize.InputRowCount" -> "5000" // mixed case
    )
