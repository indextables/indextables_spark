 ...
 * The V2 DataSource API recognizes optimizeWrite configuration but cannot actually
 * modify the execution plan to repartition data. This is a fundamental limitation
 * of the V2 API compared to V1. The configuration is parsed and logged, but the
 * actual partitioning follows Spark's default behavior.
 * 
 ...
 * 1. V2 correctly recognizes and processes optimizeWrite configuration
 * 2. Configuration hierarchy works (write options > session > table properties)
 * 3. Data integrity is maintained even without actual repartitioning
 */
class V2OptimizeWriteTest extends TestBase {
  ...
  test("V2 DataSource should support optimizeWrite with proper repartitioning") {
    ...
    df.write
      .format("com.tantivy4spark.core.Tantivy4SparkTableProvider")  // Use V2 API
      .mode("overwrite")
    ...
    assert(splitFiles.length >= 1, 
      s"Should create at least 1 split file, but found ${splitFiles.length}")
  ...
  test("V2 DataSource should respect optimizeWrite disabled setting") {
    ...
    val df = spark.range(0, 10000, 1, numPartitions = 10)
      .selectExpr(
        "id",
    ...
    assert(splitFiles.length >= 1, 
      s"Should create at least 1 split file, found ${splitFiles.length}")
