...
class MetadataColumnTest extends TestBase {
  ...
  test("should expose _indexall metadata column in table") {
    withTempPath { tempPath =>
      ...
      assert(!regularSchema.fieldNames.contains("_indexall"), 
  ...
  test("should verify metadata column implementation exists") {
    withTempPath { tempPath =>
      ...
      assert(table.isInstanceOf[org.apache.spark.sql.connector.catalog.SupportsMetadataColumns], 
      ...
      assert(metadataColumns(0).name() == "_indexall", s"Expected _indexall metadata column, got ${metadataColumns(0).name()}")
      assert(metadataColumns(0).dataType() == org.apache.spark.sql.types.StringType, "Expected StringType for _indexall")
      
      println(s"✅ Metadata column _indexall properly defined: ${metadataColumns(0).name()} (${metadataColumns(0).dataType()})")
    }
  }
}