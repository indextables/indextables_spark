...
class SqlVsDataFrameLikeTest extends TestBase {
  ...
  private def analyzePushdown(queryName: String, queryExecution: org.apache.spark.sql.execution.QueryExecution): Unit = {
  ...
  test("compare SQL LIKE vs DataFrame operations pushdown") {
    ...
    withTempPath { testPath =>
      ...
      val complexSql = spark.sql("SELECT * FROM review_table WHERE review_text LIKE '%dog%' AND category = 'animal_story'")
      ...
      complexResults.foreach(row => println(s"  - ${row.getAs[String]("review_text")} [${row.getAs[String]("category")}]"))
      ...
      complexDfResults.foreach(row => println(s"  - ${row.getAs[String]("review_text")} [${row.getAs[String]("category")}]"))
