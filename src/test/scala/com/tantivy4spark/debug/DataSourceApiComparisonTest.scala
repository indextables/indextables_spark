...
class DataSourceApiComparisonTest extends TestBase {
  ...
  test("compare DataSource V1 vs V2 API usage for SQL vs DataFrame") {
    ...
    withTempPath { testPath =>
      ...
      if (dfResults.length == sqlResults.length && sqlResults.length == dfResultsV2.length && dfResultsV2.length == sqlResultsV2.length) {
  ...
  test("investigate specific LIKE operation performance") {
    ...
    withTempPath { testPath =>
      ...
      val testData = (1 to 1000).map { i =>
        val text = if (i % 10 == 0) s"Document $i contains dog information" 
