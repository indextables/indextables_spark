...
/**
 * Isolated test for the failing "should handle date and timestamp queries" test.
 * This helps debug date/timestamp type conversions and query handling.
 */
class DateAndTimestampQueriesTest extends TestBase {
  ...
  private def createDateTimeTestDataFrame(): DataFrame = {
    ...
    spark.createDataFrame(dateTimeData).toDF(
      "id", "user_name", "event_type", "created_date", "created_timestamp", "year", "month", "day"
    )
  }
  ...
  test("should handle date and timestamp queries") {
    ...
    withTempPath { tempPath =>
      ...
      dateQueries.zipWithIndex.foreach { case (query, index) =>
