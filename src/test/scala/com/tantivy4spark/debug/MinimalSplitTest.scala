      ...
      try {
        ...
        for (i <- 0 until hits.size()) {
          ...
          if (document != null) {
            ...
            assert(titleValue.toString.startsWith("Document Title"), s"Title should start with 'Document Title', got: ${titleValue}")
            assert(contentValue.toString.contains("test content"), s"Content should contain 'test content', got: ${contentValue}")
