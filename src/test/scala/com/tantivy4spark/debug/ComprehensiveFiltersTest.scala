...
/**
 * Isolated test for the failing "should complete full write/read/query cycle with comprehensive filters" test.
 * This helps debug the full end-to-end filtering workflow and comprehensive query execution.
 */
  ...
  test("should complete full write/read/query cycle with comprehensive filters") {
    ...
    withTempPath { tempPath =>
      ...
      readData.filter(col("department") === "Engineering")
        .groupBy("location")
        .agg(count("*").alias("count"), avg("salary").alias("avg_salary")).printSchema()
