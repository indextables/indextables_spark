  ...
  test("should validate cross-field search with _indexall indexquery vs single field searches") {
    withTempPath { testDataPath =>
      ...
      val testData = Seq(
        ...
        ("doc2", "machine learning", "data processing with apache spark", "artificial intelligence", "algorithms and models"),
      ...
      spark.sql(s"CREATE OR REPLACE TEMPORARY VIEW tantivy_table USING com.tantivy4spark.core.Tantivy4SparkTableProvider OPTIONS (path '$testDataPath')")
      ...
      val titleMatches = spark.sql(s"SELECT id FROM tantivy_table WHERE title indexquery '$searchTerm'").collect().map(_.getString(0)).sorted
      val descMatches = spark.sql(s"SELECT id FROM tantivy_table WHERE description indexquery '$searchTerm'").collect().map(_.getString(0)).sorted  
      val categoryMatches = spark.sql(s"SELECT id FROM tantivy_table WHERE category indexquery '$searchTerm'").collect().map(_.getString(0)).sorted
      val tagsMatches = spark.sql(s"SELECT id FROM tantivy_table WHERE tags indexquery '$searchTerm'").collect().map(_.getString(0)).sorted
      ...
      println(s"_indexall indexquery '$searchTerm': ${indexAllMatches.mkString(", ")} (total: ${indexAllMatches.length})")
      
      // Validation: _indexall should find at least as many matches as union of all fields
      assert(indexAllMatches.length >= allFieldMatches.length, 
      ...
      allFieldMatches.foreach { expectedId =>
        assert(indexAllMatches.contains(expectedId), 
      ...
      println(s"Single field (title) matches: ${singleFieldMatches.mkString(", ")} (total: ${singleFieldMatches.length})")
