...
class PartitionedTableTest extends TestBase with BeforeAndAfterEach {
  ...
  test("should prune partitions based on filters") {
    val addActions = Seq(
      AddAction("year=2022/file1.split", Map("year" -> "2022"), 1000L, System.currentTimeMillis(), true, numRecords = Some(100L)),
      AddAction("year=2023/file2.split", Map("year" -> "2023"), 1000L, System.currentTimeMillis(), true, numRecords = Some(200L)),
      AddAction("year=2024/file3.split", Map("year" -> "2024"), 1000L, System.currentTimeMillis(), true, numRecords = Some(150L))
    )
    
    val partitionColumns = Seq("year")
    
    // Test equality filter
    val equalFilter: Array[org.apache.spark.sql.sources.Filter] = Array(org.apache.spark.sql.sources.EqualTo("year", "2023"))
    ...
    // Test IN filter
    val inFilter: Array[org.apache.spark.sql.sources.Filter] = Array(org.apache.spark.sql.sources.In("year", Array("2022", "2024")))
    ...
    val gtFilter: Array[org.apache.spark.sql.sources.Filter] = Array(org.apache.spark.sql.sources.GreaterThan("year", "2022"))
  ...
  test("should handle multiple partition columns") {
    val addActions = Seq(
      AddAction("year=2023/month=01/file1.split", Map("year" -> "2023", "month" -> "01"), 1000L, System.currentTimeMillis(), true),
      AddAction("year=2023/month=02/file2.split", Map("year" -> "2023", "month" -> "02"), 1000L, System.currentTimeMillis(), true),
      AddAction("year=2024/month=01/file3.split", Map("year" -> "2024", "month" -> "01"), 1000L, System.currentTimeMillis(), true)
    )
  ...
  test("should support overwrite mode with partitions") {
    ...
    val initialActions = Seq(
      AddAction("year=2022/file1.split", Map("year" -> "2022"), 1000L, System.currentTimeMillis(), true, numRecords = Some(50L)),
      AddAction("year=2023/file2.split", Map("year" -> "2023"), 1000L, System.currentTimeMillis(), true, numRecords = Some(75L))
    )
    transactionLog.addFiles(initialActions)
    
    // Overwrite with new partitioned data
    val overwriteActions = Seq(
      AddAction("year=2023/file3.split", Map("year" -> "2023"), 2000L, System.currentTimeMillis(), true, numRecords = Some(100L)),
      AddAction("year=2024/file4.split", Map("year" -> "2024"), 1500L, System.currentTimeMillis(), true, numRecords = Some(125L))
    )
  ...
  test("should generate partition statistics") {
    val addActions = Seq(
      AddAction("year=2023/month=01/file1.split", Map("year" -> "2023", "month" -> "01"), 1000L, System.currentTimeMillis(), true),
      AddAction("year=2023/month=01/file2.split", Map("year" -> "2023", "month" -> "01"), 1000L, System.currentTimeMillis(), true),
      AddAction("year=2023/month=02/file3.split", Map("year" -> "2023", "month" -> "02"), 1000L, System.currentTimeMillis(), true),
      AddAction("year=2024/month=01/file4.split", Map("year" -> "2024", "month" -> "01"), 1000L, System.currentTimeMillis(), true)
    )
