...
class TransactionLogEnhancedTest extends TestBase with BeforeAndAfterEach {
  ...
  test("should atomically commit transaction log only after all split files exist") {
    ...
    val readDf = spark.read.format("com.tantivy4spark.core.Tantivy4SparkTableProvider").load(testPath)
  ...
  test("should correctly read data after overwrite operation") {
    ...
    // Initial write
    val df1 = spark.range(50).toDF("id")
    ...
    // Verify initial data
    val read1 = spark.read.format("com.tantivy4spark.core.Tantivy4SparkTableProvider").load(testPath)
    assert(read1.count() == 50)
    assert(read1.filter(col("type") === "initial").count() == 50)
    
    // Overwrite with different data
    val df2 = spark.range(100, 200).toDF("id")
    ...
    val read2 = spark.read.format("com.tantivy4spark.core.Tantivy4SparkTableProvider").load(testPath)
  ...
  test("should correctly read data after multiple append operations") {
    ...
    // First write
    val df1 = spark.range(0, 25).toDF("id")
    ...
    // Second append
    val df2 = spark.range(25, 50).toDF("id")
    ...
    // Third append
    val df3 = spark.range(50, 100).toDF("id")
