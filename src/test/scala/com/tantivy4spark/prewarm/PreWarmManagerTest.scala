...
class PreWarmManagerTest extends AnyFunSuite with Matchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  test("Pre-warm explicitly disabled should skip cache warming") {
    ...
    val addActions = Seq(
      AddAction(path = "test-split-1.split", partitionValues = Map.empty, size = 1000, 
                modificationTime = System.currentTimeMillis(), dataChange = true),
      AddAction(path = "test-split-2.split", partitionValues = Map.empty, size = 2000, 
                modificationTime = System.currentTimeMillis(), dataChange = true)
    )
  ...
  test("clearAll should reset all pre-warm state") {
    ...
    val addActions = Seq(
      AddAction(path = "test-split.split", partitionValues = Map.empty, size = 1000, 
                modificationTime = System.currentTimeMillis(), dataChange = true)
    )
  ...
  test("getPreWarmStats should return statistics for executed pre-warms") {
    ...
    val queryHash = java.util.UUID.nameUUIDFromBytes(
