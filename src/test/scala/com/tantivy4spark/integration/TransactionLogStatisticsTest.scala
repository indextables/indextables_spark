...
/**
 * Integration tests specifically focused on verifying that statistics
 * are correctly written to and read from the transaction log.
 */
class TransactionLogStatisticsTest extends TestBase with BeforeAndAfterEach {
  ...
  test("should write complete statistics for all supported data types") {
    ...
    val complexData = Seq(
      (1L, "Alice", 25, 75000.50, true, java.sql.Date.valueOf("2023-01-15"), java.sql.Timestamp.valueOf("2023-01-15 10:30:00")),
      (2L, "Bob", 30, 80000.75, false, java.sql.Date.valueOf("2023-02-20"), java.sql.Timestamp.valueOf("2023-02-20 14:45:00")),
      (3L, "Charlie", 35, 85000.25, true, java.sql.Date.valueOf("2023-03-10"), java.sql.Timestamp.valueOf("2023-03-10 09:15:00")),
      (4L, "Diana", 28, 78000.00, false, java.sql.Date.valueOf("2023-04-05"), java.sql.Timestamp.valueOf("2023-04-05 16:20:00"))
    ).toDF("id", "name", "age", "salary", "active", "hire_date", "last_login")
    
    println("📊 Writing data with multiple data types...")
    
    complexData.coalesce(1).write
  ...
  test("should handle statistics for files with identical values") {
    ...
    assert(minVals("name") == "SameName" && maxVals("name") == "SameName",
      "Uniform string column should have min=max")
    assert(minVals("age") == "30" && maxVals("age") == "30",
      "Uniform int column should have min=max")
    assert(minVals("salary") == "75000.0" && maxVals("salary") == "75000.0",
      "Uniform double column should have min=max")
    assert(minVals("department") == "Engineering" && maxVals("department") == "Engineering",
      "Uniform string column should have min=max")
    
    // But id should still have different min/max
    assert(minVals("id") == "1" && maxVals("id") == "4",
      "Varying column should have different min/max")
  ...
  private def createBatchData(idStart: Long, idEnd: Long, batchName: String): DataFrame = {
    val sparkImplicits = spark.implicits
    import sparkImplicits._
  ...
  private def verifyBatchStatistics(
    ...
    assert(minVals("id") == expectedMinId.toString, 
      s"$description: expected id min=$expectedMinId, got ${minVals("id")}")
    assert(maxVals("id") == expectedMaxId.toString,
      s"$description: expected id max=$expectedMaxId, got ${maxVals("id")}")
    assert(minVals("batch") == expectedBatch,
      s"$description: expected batch=$expectedBatch, got ${minVals("batch")}")
    assert(maxVals("batch") == expectedBatch,
      s"$description: expected batch=$expectedBatch, got ${maxVals("batch")}")
    
    val expectedCount = expectedMaxId - expectedMinId + 1L
    assert(addAction.numRecords.get.toString.toLong == expectedCount,
