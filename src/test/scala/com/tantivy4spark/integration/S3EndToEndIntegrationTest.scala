 ...
 * 1. Load AWS credentials from ~/.aws/credentials
 * 2. Write data to S3 bucket s3://tantivy4java-testing
 * 3. Read data back from S3
 * 4. Perform searches and queries
...
class S3EndToEndIntegrationTest extends TestBase with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  case class AWSCredentials(accessKey: String, secretKey: String, sessionToken: Option[String] = None)
  ...
  ignore("S3 integration: Cache statistics monitoring") {
    ...
    assert(cacheTypes.exists(expectedTypes.contains), 
      s"Should have expected cache types. Found: ${cacheTypes.mkString(", ")}")
    
    // Verify schema
    val schema = cacheStats.schema
    val expectedColumns = Seq("executor_id", "hostname", "cache_type", "hits", "misses", 
                             "hit_rate", "evictions", "size_bytes", "size_mb")
  ...
  private def generateLargeTestDataset(numRecords: Int): DataFrame = {
    ...
    spark.createDataFrame(spark.sparkContext.parallelize(data)).toDF("id", "title", "content", "category", "status", "created_at", "score")
  }
