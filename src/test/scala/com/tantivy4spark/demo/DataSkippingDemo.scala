 ...
 * 
 * This demo shows how the query engine automatically skips reading files
 * that cannot contain matching data based on min/max statistics.
 */
object DataSkippingDemo extends TestBase {
  ...
  private def demonstrateDataSkipping(): Unit = {
    ...
    val files = Seq(
      ("Q1_orders.tnt4s", Map("customer_id" -> "1", "order_amount" -> "10.50", "region" -> "East"), 
                         Map("customer_id" -> "1000", "order_amount" -> "999.99", "region" -> "East")),
      ("Q2_orders.tnt4s", Map("customer_id" -> "1001", "order_amount" -> "25.00", "region" -> "West"),
                         Map("customer_id" -> "2000", "order_amount" -> "1500.00", "region" -> "West")),
      ("Q3_orders.tnt4s", Map("customer_id" -> "2001", "order_amount" -> "15.75", "region" -> "North"),
                         Map("customer_id" -> "3000", "order_amount" -> "1200.50", "region" -> "North")),  
      ("Q4_orders.tnt4s", Map("customer_id" -> "3001", "order_amount" -> "5.99", "region" -> "South"),
                         Map("customer_id" -> "4000", "order_amount" -> "2000.00", "region" -> "South")),
      ("Special_orders.tnt4s", Map("customer_id" -> "10001", "order_amount" -> "50000.00", "region" -> "Global"),
                               Map("customer_id" -> "10010", "order_amount" -> "100000.00", "region" -> "Global"))
    )
    
    files.foreach { case (filename, minVals, maxVals) =>
      ...
      println(s"   ✓ $filename: customer_id [${minVals("customer_id")}-${maxVals("customer_id")}], " +
  ...
  private def demonstrateScenario1_EqualTo(transactionLog: TransactionLog): Unit = {
    ...
    println(s"   💡 Skipped ${totalFiles - scannedFiles} files (${((totalFiles - scannedFiles) * 100.0 / totalFiles).toInt}% reduction)")
