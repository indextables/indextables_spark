...
class ComplexNotFilterTest extends TestBase {
  ...
  test("Complex NOT filter validation with comprehensive test data") {
    ...
    test1Results.foreach(row => println(s"  ID=${row.getInt(0)}: ${row.getString(1)}, available=${row.getBoolean(2)}, price=${row.getInt(3)}"))
    ...
    test1Results.foreach { row =>
      ...
      assert(available || price != 100, s"Record should have available=true OR price!=100, but available=$available, price=$price")
    }
    ...
    test2Results.foreach(row => println(s"  ID=${row.getInt(0)}: ${row.getString(1)}, available=${row.getBoolean(2)}, price=${row.getInt(3)}"))
    ...
    test3Results.foreach(row => println(s"  ID=${row.getInt(0)}: ${row.getString(1)}, available=${row.getBoolean(2)}, price=${row.getInt(3)}"))
