...
class NotFilterTest extends TestBase {
  ...
  test("SQL NOT filter validation with IndexQuery combinations") {
    ...
    val notElectronicsResults = spark.sql("""
    ...
    assert(actualNotElectronics == expectedNotElectronics, s"Expected $expectedNotElectronics but got $actualNotElectronics")
    ...
    val notHighPriceResults = spark.sql("""
    ...
    val indexQueryWithNotResults = spark.sql("""
    ...
    if (actualIndexQueryNot.nonEmpty) {
      ...
      assert(actualIndexQueryNot == expectedIndexQueryNot, s"Expected $expectedIndexQueryNot but got $actualIndexQueryNot")
    ...
    val complexNotResults = spark.sql("""
    ...
    complexNotResults.foreach(row => println(s"  ID=${row.getInt(0)}: ${row.getString(1)}, available=${row.getBoolean(2)}, price=${row.getInt(3)}"))
    ...
    complexNotResults.foreach { row =>
      ...
      assert(available || price >= 100, s"Should be available=true OR price>=100, but available=$available, price=$price")
    }
