...
/**
 * SQL-based validation test for IndexQuery operations using valid Quickwit syntax.
 * Tests various query types via SQL interface and validates correct results.
 */
class SQLIndexQueryValidationTest extends TestBase {
  ...
  test("Debug current data and IndexQuery behavior") {
    ...
    allData.foreach(row => println(s"  ID=${row.getInt(0)}: title='${row.getString(1)}', category='${row.getString(2)}', tags='${row.getString(3)}'"))
  ...
  test("Demonstrate AND/OR pushdown composition into Quickwit queries") {
    ...
    println("\n🔍 Test 3: Complex boolean composition")
    val complexBooleanResults = spark.sql("""
      SELECT id, title, category FROM tantivy_documents
    ...
    val multiIndexQueryResults = spark.sql("""
  ...
  test("String field pushdown with simple exact matching") {
    ...
    val multiCategoryResults = spark.sql("""
      SELECT id, category FROM tantivy_documents
  ...
  test("Numeric field range queries and pushdown") {
    ...
    println("\n🔍 Numeric range query")
    val scoreRangeResults = spark.sql("""
    ...
    println("\n🔍 Exact numeric match")
    val specificScoreResults = spark.sql("""
      SELECT id, score FROM tantivy_documents
  ...
  test("Complex mixed WHERE clauses with AND/OR combinations") {
    ...
    val complexNestedResults = spark.sql("""
      SELECT id, title, category, score FROM tantivy_documents
    ...
    val indexQueryOrComplexResults = spark.sql("""
      SELECT id, title, category, score FROM tantivy_documents
    ...
    val deeplyNestedResults = spark.sql("""
  ...
  test("IndexQuery + equals/not-equals filters with result validation") {
    ...
    val machineAndTechResults = spark.sql("""
      SELECT id, title, category FROM tantivy_documents
      WHERE title indexquery 'machine' AND category = 'tech'
    """).collect()

    println(s"Results: ${machineAndTechResults.length}")
    machineAndTechResults.foreach(row => println(s"  ID=${row.getInt(0)}: title='${row.getString(1)}', category='${row.getString(2)}'"))
    ...
    val complexResults = spark.sql("""
      SELECT id, title, category, score, featured FROM tantivy_documents
      WHERE title indexquery 'machine OR data' AND score >= 90 AND featured = true
    """).collect()

    println(s"Results: ${complexResults.length}")
    complexResults.foreach(row => println(s"  ID=${row.getInt(0)}: title='${row.getString(1)}', score=${row.getInt(3)}, featured=${row.getBoolean(4)}"))
  ...
  test("Comprehensive false positive/negative validation") {
    ...
    val baselineDataResults = spark.sql("""
      SELECT id, title FROM tantivy_documents
    ...
    // Now test IndexQuery
    val indexQueryDataResults = spark.sql("""
      SELECT id, title FROM tantivy_documents
      WHERE title indexquery 'data'
    """).collect()
    ...
    val nonexistentResults = spark.sql("""
      SELECT id, title FROM tantivy_documents
      WHERE title indexquery 'nonexistent'
    """).collect()

    assert(nonexistentResults.length == 0, s"Should find 0 results for 'nonexistent' but found ${nonexistentResults.length}")
    ...
    val precisionResults = spark.sql("""
      SELECT id, title, category, score FROM tantivy_documents
    ...
    precisionResults.foreach(row => println(s"  ID=${row.getInt(0)}: title='${row.getString(1)}', category='${row.getString(2)}', score=${row.getInt(3)}"))
  ...
  test("End-to-end validation with complete result verification") {
    ...
    complexQuery.foreach(row => println(s"  ID=${row.getInt(0)}: title='${row.getString(1)}', category='${row.getString(2)}', score=${row.getInt(3)}, featured=${row.getBoolean(4)}"))
