...
class OptimizedTransactionLog(
      ...
      detectedAvroFormat match {
        ...
        case None         =>
          ...
          explicitFormat match {
            case Some("json") =>
              // Explicitly configured for JSON
              detectedAvroFormat = Some(false)
              return false
            case Some("avro") =>
              // Explicitly configured for Avro
              detectedAvroFormat = Some(true)
              return true
            case _ =>
              // Not explicitly configured - auto-detect or default to Avro
              // Check if Avro state already exists (from a previous checkpoint)
              val hasAvroState = checkpoint.flatMap(_.getLastCheckpointInfo()).exists { info =>
                info.format.contains(StateConfig.Format.AVRO_STATE)
              }

              if (hasAvroState) {
                logger.info("Auto-detected Avro state format from existing checkpoint")
                detectedAvroFormat = Some(true)
                return true
              }

              // Default to Avro for new tables
              detectedAvroFormat = Some(true)
              return true
          }
      }
  ...
  /**
   * Get the cache file path for a transaction log file. Extracts the filename from the cloud path to use as the local
   * cache filename.
   */
  ...
  /**
   * Clear all cached transaction log files for this table. Called when cache validation detects a size mismatch,
   * indicating the table may have been deleted and recreated with the same path.
   */
  private def clearTableCache(): Unit = {
    val files = txlogCacheDir.listFiles()
    if (files != null) {
      var deletedCount = 0
      files.foreach(file => if (file.delete()) deletedCount += 1)
  ...
  /**
   * Add files using retry-enabled write for concurrent safety.
   *
  ...
  def addFiles(addActions: Seq[AddAction]): Long = {
    ...
    val version = writeActionsWithRetry(addActions)
  ...
  def removeFile(path: String, deletionTimestamp: Long = System.currentTimeMillis()): Long = {
    logger.debug(s" Removing file '$path'")
    ...
    // Use writeActionsWithRetry to properly route to Avro when enabled
    val version = writeActionsWithRetry(Seq(removeAction))
  ...
  /**
   * Overwrite files with retry on concurrent write conflict.
   *
   * On each retry attempt, the list of files to remove is recalculated to include any files that may have been added by
   * competing writers.
   */
  def overwriteFiles(addActions: Seq[AddAction]): Long = {
    ...
    // Route to Avro state format if enabled
    if (isAvroFormatEnabled()) {
      logger.debug(s"Avro mode: routing overwriteFiles through writeActionsToAvroState")
      ...
      logger.info(s"Avro overwrite complete: removed ${removeActions.length} files, added ${addActions.length} files")
      return version
    }

    // JSON format - existing logic
    ...
    while (attempt <= retryConfig.maxAttempts) {
      ...
      // Recalculate files to remove on each attempt
      val existingFiles = listFilesOptimized()
      logger.debug(s" Found ${existingFiles.size} existing files to remove")
      val removeActions = existingFiles.map { file =>
        RemoveAction(
          path = file.path,
          deletionTimestamp = Some(System.currentTimeMillis()),
      ...
      if (writeSucceeded) {
        ...
        if (attempt > 1) {
          logger.info(s"Overwrite complete: removed ${removeActions.length} files, added ${addActions.length} files after $attempt attempts")
        } else {
          logger.info(s"Overwrite complete: removed ${removeActions.length} files, added ${addActions.length} files")
        }
      ...
      logger.warn(s"Concurrent write conflict during overwrite at version $version (attempt $attempt/${retryConfig.maxAttempts}). Retrying.")
  ...
  /**
   * Commits a merge splits operation atomically with retry on concurrent conflict.
   *
   * Merge operations are idempotent since RemoveActions for already-removed files are no-ops.
   */
  ...
  override def commitSyncActions(
    ...
    try {
      ...
      logger.info(
        s"commitSyncActions: re-cached metadata for ${tablePath.toString}, " +
  ...
  override def listFilesWithPartitionFilters(partitionFilters: Seq[Filter]): Seq[AddAction] = {
    ...
    // Check if checkpoint is in Avro format (no need to read JSON versions)
    // Use cached version to avoid repeated storage calls
    val isAvroCheckpoint = getLastCheckpointInfoCached().exists { info =>
      info.format.contains(StateConfig.Format.AVRO_STATE)
    }
  ...
  private def listFilesOptimized(): Seq[AddAction] = {
    // For Avro mode, read directly from Avro state to avoid caching issues
    // The Avro state is the complete source of truth in pure Avro mode
  ...
  def getMetadata(): MetadataAction =
    enhancedCache.getOrComputeMetadata(
      tablePath.toString, {
        ...
        logger.info(
          s"getMetadata: checkpointInfo=${checkpointInfoOpt.map(i => s"v${i.version},format=${i.format},stateDir=${i.stateDir}").getOrElse("NONE")}"
        )
        ...
        val baseMetadata: Option[MetadataAction] = getCheckpointActionsCached().flatMap { checkpointActions =>
          val actionTypes =
            checkpointActions.map(_.getClass.getSimpleName).groupBy(identity).map { case (k, v) => s"$k:${v.size}" }
          ...
          checkpointActions.collectFirst { case metadata: MetadataAction => metadata }.map { metadata =>
            ...
            logger.info(
              s"Found metadata in checkpoint (version ${checkpointVersion.getOrElse("unknown")}): " +
  ...
  override def getVersions(): Seq[Long] = {
    ...
    files.foreach(file => knownFileSizes.put(file.path, file.size))
   ...
   *
   * This method downloads the file to local temp storage first, then reads from local disk. This avoids the high
   * per-line latency of streaming directly from object storage (S3/Azure), where each BufferedReader.readLine() call
   * incurs network round-trip overhead.
   *
  ...
  /**
   * Download a transaction log file from cloud storage to the local cache. Uses Java NIO Files.copy() which is
   * optimized for stream-to-file transfers:
  ...
  /** Read actions from a local cached file. Handles decompression transparently. */
  ...
  private def applySchemaDeduplicationForWrite(
    ...
    if (newSchemas.nonEmpty) {
      ...
      val currentMetadata = actions
        .collectFirst { case m: MetadataAction => m }
        .getOrElse {
          try
            getMetadata()
          catch {
  ...
  private def writeActionsToAvroState(actions: Seq[Action]): Long = {
    ...
    inputMetadata.foreach { m =>
      ...
      logger.info(
        s"writeActionsToAvroState: MetadataAction found with companion.enabled=$companionEnabled, " +
    ...
    val metadataJson = actions.collectFirst {
      case m: MetadataAction =>
        val node = JsonUtil.mapper.createObjectNode()
        node.set[com.fasterxml.jackson.databind.JsonNode]("metaData", JsonUtil.mapper.valueToTree(m))
    ...
    // Invalidate all caches since we wrote a new state
    enhancedCache.invalidateVersionDependentCaches(tablePath.toString)
    enhancedCache.invalidateLastCheckpointInfo(tablePath.toString)
    enhancedCache.invalidateCheckpointActions(tablePath.toString, actualVersion)
  ...
  private def writeActionsWithRetry(actions: Seq[Action]): Long = {
    // Route to Avro state format if enabled
    if (isAvroFormatEnabled()) {
      return writeActionsToAvroState(actions)
    }
    ...
    while (attempt <= retryConfig.maxAttempts) {
      ...
      logger.debug(s" Writing version $version to $versionFilePath with ${actions.size} actions (attempt $attempt)")
      ...
      if (writeSucceeded) {
        ...
        val addActionsToWrite = actionsToWrite.collect { case add: AddAction => add }
        if (addActionsToWrite.nonEmpty) {
          val currentFiles =
            try listFiles()
            catch { case _: Exception => Seq.empty[AddAction] }
        ...
        if (attempt > 1) {
          logger.info(
            s"Written ${actionsToWrite.length} actions to version $version$compressionInfo after $attempt attempts"
          )
        } else {
          logger.info(s"Writing ${actionsToWrite.length} actions to version $version$compressionInfo: ${actionsToWrite.map(_.getClass.getSimpleName).mkString(", ")}")
        }
      ...
      conflictedVersions += version
      logger.warn(
        s"Concurrent write conflict at version $version (attempt $attempt/${retryConfig.maxAttempts}). Retrying."
      )
    ...
    throw new TransactionConflictException(
      s"Failed to write transaction log after ${retryConfig.maxAttempts} attempts. Last conflicted version: $lastConflictVersion.",
      lastConflictVersion,
      retryConfig.maxAttempts
    )
  }
   ...
   *
   * Used for initialization (version 0) where retry doesn't make sense.
   */
  ...
  private def reconstructStateStandard(versions: Seq[Long]): Seq[AddAction] = {
    ...
    val (startVersion, versionsToProcess) = checkpointInfo.map(_.version) match {
      case Some(checkpointVersion) if versions.contains(checkpointVersion) || isAvroCheckpoint =>
        logger.info(s"Starting reconstruction from checkpoint at version $checkpointVersion (avro=$isAvroCheckpoint)")
        ...
        // For Avro checkpoints, don't process any JSON versions - Avro state IS the complete state
        // For JSON checkpoints, process versions after checkpoint
        if (isAvroCheckpoint) {
          (checkpointVersion, Seq.empty)
        } else {
  ...
  private def computeCurrentChecksumWithVersions(versions: Seq[Long]): String = {
    ...
    // For Avro mode, also include the Avro state version to ensure cache invalidation
    // when new Avro states are written (since there are no JSON version files in pure Avro mode)
    val avroStateVersion = getLastCheckpointInfoCached()
      .filter(_.format.contains(StateConfig.Format.AVRO_STATE))
      .map(_.version)
      .getOrElse(0L)

    s"${tablePath.toString}-$versionHash-avro$avroStateVersion"
  }
  ...
  /**
   * Get all SkipActions from the transaction log using cached checkpoint data. This is more efficient than the base
   * TransactionLog implementation because it reuses the cached checkpoint actions.
   */
  ...
  /** Get files currently in cooldown period. Returns a map of file path to retry-after timestamp. */
  ...
  /**
   * Filter out files that are in cooldown from a list of candidate files for merge. Uses cached checkpoint data for
   * efficiency.
   */
