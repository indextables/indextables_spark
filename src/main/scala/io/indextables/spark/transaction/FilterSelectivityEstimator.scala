...
/**
 * Estimates filter selectivity for optimal filter ordering.
 * More selective filters (lower score) are evaluated first to enable
 * early termination in AND conditions.
 *
...
object FilterSelectivityEstimator {
   ...
   * @return Selectivity score (lower = more selective)
   */
  ...
  /**
   * Order filters by selectivity for optimal evaluation.
   * Most selective filters (lowest score) come first.
   *
   * @param filters Array of filters to order
   * @return Array of filters ordered by selectivity (most selective first)
   */
  def orderBySelectivity(filters: Array[Filter]): Array[Filter] = {
    if (filters.length <= 1) {
      ...
      logger.debug(s"Ordered ${filters.length} filters by selectivity: " +
  ...
  /**
   * Check if a filter can use the partition index for O(1) lookup.
   * Only equality and small IN filters qualify.
   *
   * @param filter The filter to check
   * @return true if the filter can use index lookup
   */
   ...
   * @return Tuple of (indexable filters, remaining filters)
   */
