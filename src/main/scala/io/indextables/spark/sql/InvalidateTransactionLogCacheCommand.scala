  ...
  override def run(sparkSession: SparkSession): Seq[Row] =
    tablePath match {
      ...
      case None =>
        ...
        val (actionsStats, checkpointStats, avroManifestStats, avroFileListStats, filteredSchemaStats, avroManifestFileStats) = io.indextables.spark.transaction.EnhancedTransactionLogCache.getGlobalCacheStats()
        val hitsBefore = actionsStats.hitCount() + checkpointStats.hitCount() + avroManifestStats.hitCount() + avroFileListStats.hitCount() + filteredSchemaStats.hitCount() + avroManifestFileStats.hitCount()
        val missesBefore = actionsStats.missCount() + checkpointStats.missCount() + avroManifestStats.missCount() + avroFileListStats.missCount() + filteredSchemaStats.missCount() + avroManifestFileStats.missCount()
