 ...
 *
 * Purge-on-write automatically cleans up orphaned split files and old transaction logs
 * after write operations, similar to merge-on-write.
 *
 ...
 *   - spark.indextables.purgeOnWrite.txLogRetentionHours: Transaction log retention period in hours (default: 720 = 30 days)
 *
 * Trigger modes:
 *   1. After merge-on-write: Purge runs when merge-on-write completes (if triggerAfterMerge=true)
 *   2. After N writes: Purge runs every N write operations on the table (if triggerAfterWrites > 0)
 *
 * Safety features:
...
object PurgeOnWriteConfig {
  ...
  def fromOptions(options: CaseInsensitiveStringMap): PurgeOnWriteConfig = {
    ...
    val splitRetentionHours = try {
      options.getOrDefault("spark.indextables.purgeOnWrite.splitRetentionHours", DEFAULT_SPLIT_RETENTION_HOURS.toString).toLong
    } catch {
      case e: NumberFormatException =>
    ...
    val txLogRetentionHours = try {
      options.getOrDefault("spark.indextables.purgeOnWrite.txLogRetentionHours", DEFAULT_TX_LOG_RETENTION_HOURS.toString).toLong
    } catch {
      case e: NumberFormatException =>
  ...
  def validate(config: PurgeOnWriteConfig, retentionCheckEnabled: Boolean = true): Unit = {
    ...
    require(config.txLogRetentionHours >= 0, s"Transaction log retention cannot be negative: ${config.txLogRetentionHours}")
