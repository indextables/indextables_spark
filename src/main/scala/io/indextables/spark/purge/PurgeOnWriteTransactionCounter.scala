 ...
 *
 * Tracks the number of write transactions completed on each table path within
 * a Spark session. When the count reaches the configured threshold, purge-on-write
 * is triggered and the counter resets.
 *
 * This is a session-scoped singleton - each Spark session maintains its own counter state.
 * Counters are NOT persisted across sessions or JVM restarts.
 *
...
object PurgeOnWriteTransactionCounter {
  ...
  /**
   * Per-table transaction counters.
   * Key: table path (normalized)
   * Value: atomic counter for number of transactions completed
   */
   ...
   *
   * @param tablePath The table path (will be normalized)
   * @return The new transaction count for this table
   */
   ...
   *
   * @param tablePath The table path (will be normalized)
   * @return The current transaction count, or 0 if never written
   */
  ...
  /**
   * Reset the transaction count for a table path to zero.
   * Called after purge-on-write completes.
   *
   * @param tablePath The table path (will be normalized)
   */
   ...
   * Used for testing or session cleanup.
   */
  def clearAll(): Unit = {
  ...
  /**
   * Normalize table path for consistent lookup.
   * Converts to canonical form and removes trailing slashes.
   */
