 ...
 *
 * These expressions are designed for pushdown to the tantivy4java data source.
 * They carry configuration for bucket aggregations (DateHistogram, Histogram, Range)
 * that are executed efficiently inside Quickwit/Tantivy rather than in Spark.
 */
 ...
 *
 * Usage:
 *   indextables_date_histogram(timestamp_column, '1h')
 *   indextables_date_histogram(timestamp_column, '1d', '6h')  -- with offset
 *
 * @param field The timestamp/date column to bucket
 * @param interval The bucket interval (e.g., "1h", "1d", "30m")
 * @param offset Optional offset to shift bucket boundaries (e.g., "6h" to start day at 6am)
 ...
 *
 * Usage:
 *   indextables_histogram(price, 100)           -- $100 buckets
 *   indextables_histogram(score, 10, 5)         -- buckets offset by 5
 *
 * @param field The numeric column to bucket
 * @param interval The fixed bucket width
 * @param offset Optional offset to shift bucket boundaries (default: 0.0)
 * @param minDocCount Minimum document count for bucket to be returned (default: 0)
 ...
 *
 * Usage:
 *   indextables_range(price, 'budget', NULL, 50, 'mid', 50, 200, 'premium', 200, NULL)
 *
 * @param field The numeric column to bucket
 * @param ranges Sequence of named ranges with (name, from, to)
 */
...
object BucketExpressions {
  ...
  /**
   * Extract bucket expression from a complex expression tree.
   * Returns the first bucket expression found.
   */
