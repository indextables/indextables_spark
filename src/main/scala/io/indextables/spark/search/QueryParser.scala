 ...
 *
 * This is a basic implementation that handles common query patterns. For more advanced query parsing, consider using
 * tantivy4java's Index.parseQuery method.
 */
object QueryParser {
    ...
    try {
      ...
      if (query.contains(" OR ")) {
        ...
        if (parts.length >= 2) {
          ...
          val queries = parts.map(part => parseSingleQueryPart(part, schema)).toList

          // Create OccurQuery objects for OR query (should clauses)
          val occurQueries = queries.map(query => new Query.OccurQuery(Occur.SHOULD, query))
      ...
      if (query.contains(" AND ")) {
        ...
        if (parts.length >= 2) {
          ...
          val occurQueries = queries.map(query => new Query.OccurQuery(Occur.MUST, query))
          return Query.booleanQuery(occurQueries.asJava)
        }
      }
    ...
    if ((value.startsWith("[") && value.endsWith("]")) || (value.startsWith("{") && value.endsWith("}"))) {
      ...
      if (rangeContent.contains(" TO ")) {
        ...
        if (rangeParts.length == 2) {
          ...
          logger.debug(
            s"Creating range query for field '$fieldName': $lowerBound TO $upperBound (inclusive: $isInclusive)"
          )
