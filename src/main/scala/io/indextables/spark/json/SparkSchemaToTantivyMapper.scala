...
class SparkSchemaToTantivyMapper(options: IndexTables4SparkOptions) {
   ...
   *
   * @param field Spark StructField to check
   * @return true if field should use JSON field type
   */
   ...
   *
   * @param fieldName Name of the field
   * @return true if field needs fast field configuration
   */
   ...
   *
   * @param fieldName Name of the field
   * @return JsonFieldConfig with settings
   */
 ...
 *
 * @param parseOnWrite Whether to parse JSON strings on write (default: true)
 * @param failOnInvalidJson Whether to fail on invalid JSON or store as text (default: false)
 * @param enableRangeQueries Whether to enable range queries (requires fast fields) (default: false)
 */
