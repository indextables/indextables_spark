...
class IndexTables4SparkTable(
  ...
  override def newWriteBuilder(info: LogicalWriteInfo): WriteBuilder = {
    ...
    // Keep credential provider class in config so executors can call it directly
    // to get fresh credentials at upload time (prevents expiration during long writes)
    import scala.jdk.CollectionConverters._
    val baseOptionsMap = options.asScala.toMap
