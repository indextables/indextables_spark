...
import io.indextables.tantivy4java.split.{SplitBooleanQuery, SplitMatchAllQuery, SplitQuery, SplitSearcher, SplitTermQuery}
...
object FiltersToQueryConverter {
  ...
  /**
   * Convert Spark filters to a SplitQuery using a raw SplitSearcher.
   *
   * This is a simplified version for use cases like XRef searching where we have
   * direct access to a SplitSearcher but not a full SplitSearchEngine. It skips
   * JSON field translation and Spark schema-specific handling.
   *
   * @param filters
   *   Spark filters from predicate pushdown
   * @param splitSearcher
   *   The tantivy4java SplitSearcher for query parsing
   * @return
   *   A SplitQuery representing the filter conditions
   */
  def convertToSplitQuery(filters: Array[Filter], splitSearcher: SplitSearcher): SplitQuery = {
    if (filters.isEmpty) {
      return new SplitMatchAllQuery()
    }

    queryLog(s"FiltersToQueryConverter (SplitSearcher) received ${filters.length} filters")

    // Get schema from the SplitSearcher
    val schema = splitSearcher.getSchema()
    try {
      val splitQueries = filters.flatMap { filter =>
        convertFilterToSplitQuerySimple(filter, schema, splitSearcher)
      }
  ...
  /**
   * Convert a single Spark filter to a SplitQuery using simplified logic.
   * This version works directly with SplitSearcher and Schema without SplitSearchEngine.
   */
  private def convertFilterToSplitQuerySimple(
    filter: Filter,
    schema: Schema,
    ...
    filter match {
      case EqualTo(attribute, value) =>
        // Use SplitTermQuery for exact term matching
        // First detect date/timestamp values directly - XRef schemas may report these as TEXT
        // because XRef builds merge term dictionaries without preserving original field types
        value match {
          case ts: java.sql.Timestamp =>
            // Timestamps are stored as INTEGER fields with MICROSECONDS since epoch
            // (see IndexTablesDirectInterface: builder.addIntegerField for TimestampType)
      ...
      case In(attribute, values) if values.nonEmpty =>
        // First detect date/timestamp values directly - XRef schemas may report these as TEXT
        val hasTimestamp = values.exists(_.isInstanceOf[java.sql.Timestamp])
        ...
        if (hasDateOnly) {
          ...
          values.foreach {
            case ts: java.sql.Timestamp =>
              val micros = ts.getTime * 1000L // millis to micros
              boolQuery.addShould(new SplitTermQuery(attribute, micros.toString))
            case other =>
              // Non-timestamp values in a timestamp IN clause - unusual but handle gracefully
              boolQuery.addShould(new SplitTermQuery(attribute, other.toString))
          }
          Some(boolQuery)
        } else {
          // Non-date values - use schema-based type detection
          val fieldType = getFieldType(schema, attribute)
          val boolQuery = new SplitBooleanQuery()
          values.foreach { value =>
            val convertedValue = convertSparkValueToTantivy(value, fieldType)
            boolQuery.addShould(new SplitTermQuery(attribute, convertedValue.toString))
          }
          Some(boolQuery)
        }
      ...
      case StringStartsWith(attribute, value) =>
        try {
          Some(splitSearcher.parseQuery(s"$attribute:$value*"))
        } catch {
          case e: Exception =>
            queryLog(s"Failed to create prefix query for $attribute: ${e.getMessage}")
            None
        }
      ...
      case StringContains(attribute, value) =>
        try {
          ...
          case e: Exception =>
            queryLog(s"Failed to create contains query for $attribute: ${e.getMessage}")
            None
        }
      ...
      case StringEndsWith(attribute, value) =>
        try {
          ...
          case e: Exception =>
            queryLog(s"Failed to create suffix query for $attribute: ${e.getMessage}")
            None
        }

      case And(left, right) =>
        val leftQuery = convertFilterToSplitQuerySimple(left, schema, splitSearcher)
        val rightQuery = convertFilterToSplitQuerySimple(right, schema, splitSearcher)
      ...
      case Not(child) =>
        convertFilterToSplitQuerySimple(child, schema, splitSearcher).map { cq =>
          val boolQuery = new SplitBooleanQuery()
      ...
      case GreaterThan(_, _) | GreaterThanOrEqual(_, _) | LessThan(_, _) | LessThanOrEqual(_, _) =>
        // Range queries return match-all for XRef (conservative - don't miss any splits)
        queryLog(s"Range filter $filter returns match-all (XRef doesn't support range queries)")
        Some(new SplitMatchAllQuery())

      case indexQuery: IndexQueryFilter =>
        try {
          val fieldNames = java.util.Arrays.asList(indexQuery.columnName)
          Some(splitSearcher.parseQuery(indexQuery.queryString, fieldNames))
        } catch {
          case e: Exception =>
            queryLog(s"Failed to parse IndexQuery: ${e.getMessage}")
            None
        }

      case _ =>
        queryLog(s"Unsupported filter for simple conversion: $filter")
        None
    }
