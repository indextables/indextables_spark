...
/**
 * CompanionSourceReader for bare Parquet directories (no table format).
 * Discovers parquet files via CloudStorageProvider (S3, Azure, local) and extracts
 * Hive-style partition columns from directory structure (col=value/).
 * Schema is read via tantivy4java's ParquetSchemaReader (Arrow-based, JNI).
 *
...
class ParquetDirectoryReader(
  ...
  private val nativeConfig: java.util.Map[String, String] = {
    ...
    credentials.get("spark.indextables.aws.accessKey")
      .foreach(v => config.put("aws_access_key_id", v))
    credentials.get("spark.indextables.aws.secretKey")
      .foreach(v => config.put("aws_secret_access_key", v))
    credentials.get("spark.indextables.aws.sessionToken")
      .foreach(v => config.put("aws_session_token", v))
    credentials.get("spark.indextables.aws.region")
      .foreach(v => config.put("aws_region", v))
    credentials.get("spark.indextables.azure.accountName")
      .foreach(v => config.put("azure_account_name", v))
    credentials.get("spark.indextables.azure.accountKey")
  ...
  override def schemaSourceParquetFile(): Option[String] =
    schemaSourcePath.orElse(discoveredFiles.headOption.map { f =>
      ...
      if (f.path.startsWith("/") || f.path.startsWith("s3://") || f.path.startsWith("s3a://") ||
  ...
  /**
   * Discover all *.parquet files using CloudStorageProvider for listing.
   * Extracts Hive-style partition values from the path structure.
   */
  ...
  /**
   * Extract relative path from an absolute path given the root.
   * Handles scheme differences (e.g., s3:// vs s3a://) by normalizing both paths.
   */
  ...
  /**
   * Extract Hive-style partition values from a relative path.
   * E.g., "year=2024/month=01/file.parquet" -> Map("year" -> "2024", "month" -> "01")
   */
   ...
   *
   * ParquetSchemaReader returns an Arrow-based schema JSON that isn't directly compatible
   * with Spark's DataType.fromJson(). For Spark-written parquet files, the Spark schema is
   * embedded in parquet key-value metadata under "org.apache.spark.sql.parquet.row.metadata".
   * We extract and use that if present; otherwise, we convert the Arrow format to Spark format.
   */
  ...
  /**
   * Extract Spark schema from parquet key-value metadata if present.
   * Spark writes its schema under "org.apache.spark.sql.parquet.row.metadata".
   */
  ...
  /**
   * Convert Arrow-format schema JSON to Spark StructType.
   * Arrow format: {"fields": [{"name": "...", "data_type": "int64", "nullable": ...}]}
   * Spark format: {"type": "struct", "fields": [{"name": "...", "type": "long", ...}]}
   */
  ...
  private def arrowTypeToSparkType(arrowType: String): DataType = arrowType.toLowerCase match {
    ...
    case "timestamp" | "timestamp_us" | "timestamp_ms" | "timestamp_ns" |
         "timestamp[us]" | "timestamp[ms]" | "timestamp[ns]" |
         "timestamp[us, tz=utc]" | "timestamp[ms, tz=utc]" |
         "timestamp[ns, tz=utc]" => TimestampType
  ...
  /**
   * Strip `file:` URI scheme prefix from local filesystem paths.
   * CloudStorageProvider returns paths with `file:` prefix on local filesystem,
   * but tantivy4java expects plain filesystem paths.
   */
