...
/**
 * Broadcast-based split locality manager that enables efficient distribution of 
 * cache locality information across the cluster.
 * 
 * This manager addresses the key issue where preferred locations need to be computed
 * on the driver during partition planning, but cache access patterns are observed 
 * on individual executors. By using broadcast variables, we can:
 * 
 * 1. Collect cache locality information from all executors
 * 2. Broadcast this information back to all nodes in the cluster
 * 3. Enable accurate preferredLocations() calculations during query planning
 */
object BroadcastSplitLocalityManager {
  ...
  /**
   * Record that a split has been accessed on the current host.
   * This is called from executors during split cache operations.
   */
  ...
  /**
   * Get preferred hosts for a split from the current broadcast locality information.
   * This is called during partition planning on the driver.
   */
  ...
  /**
   * Create and update the broadcast variable with current cluster-wide locality information.
   * This should be called periodically by the driver to refresh locality information.
   */
  def updateBroadcastLocality(sc: SparkContext): Broadcast[Map[String, Array[String]]] = {
    ...
    val mergedLocalityMap = executorLocalityInfo
      ...
      .view.map { case (splitPath, entries) =>
    ...
    mergedLocalityMap.foreach { case (splitPath, hosts) =>
  ...
  /**
   * Invalidate a specific split from the broadcast locality information.
   * This should be called when splits are removed or become invalid.
   */
  ...
  /**
   * Clear all locality information (both local and broadcast).
  ...
  /**
   * Force an immediate update of broadcast locality information.
