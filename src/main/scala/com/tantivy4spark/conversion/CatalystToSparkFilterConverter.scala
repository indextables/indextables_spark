...
package com.tantivy4spark.conversion
...
/**
 * Converts Catalyst expressions to Spark Filter objects for filter pushdown.
 * This enables proper handling of regular filter expressions in CatalystScan.
 */
object CatalystToSparkFilterConverter {
   ...
   * Returns Some(filter) if conversion is successful, None otherwise.
   */
  def convertExpression(expr: expressions.Expression): Option[sources.Filter] = {
    expr match {
      ...
      case expressions.GreaterThanOrEqual(expressions.AttributeReference(name, _, _, _), expressions.Literal(value, _)) =>
        Some(sources.GreaterThanOrEqual(name, value))

      case expressions.GreaterThanOrEqual(expressions.Literal(value, _), expressions.AttributeReference(name, _, _, _)) =>
      ...
      // String operations
      case expressions.Contains(expressions.AttributeReference(name, _, _, _), expressions.Literal(value, StringType)) =>
        Some(sources.StringContains(name, value.toString))

      case expressions.StartsWith(expressions.AttributeReference(name, _, _, _), expressions.Literal(value, StringType)) =>
        Some(sources.StringStartsWith(name, value.toString))

      case expressions.EndsWith(expressions.AttributeReference(name, _, _, _), expressions.Literal(value, StringType)) =>
        Some(sources.StringEndsWith(name, value.toString))

      // IN predicate
      case expressions.In(expressions.AttributeReference(name, _, _, _), values) =>
        val literalValues = values.collect {
          case expressions.Literal(value, _) => value
        }
