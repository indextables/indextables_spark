...
/**
 * Utilities for handling partition operations similar to Delta Lake.
 * Provides functionality for extracting partition values from data rows
 * and managing partition metadata.
 */
object PartitionUtils {
  ...
  /**
   * Extract partition values from a data row based on partition column indices.
   * Returns a map of partition column names to their string representations.
   */
  ...
  /**
   * Convert a partition value to its string representation for storage in transaction log.
   * This follows Spark's partition path encoding rules.
   */
  private def convertPartitionValue(row: InternalRow, index: Int, dataType: DataType): String = {
  ...
  def isSupportedPartitionType(dataType: DataType): Boolean = {
    dataType match {
      case StringType | IntegerType | LongType | FloatType | DoubleType | BooleanType |
           DateType | TimestampType | _: DecimalType => true
      case _ => false
    }
  }
  
  /**
   * Extract unique partition values from a set of rows for a given partition column.
   * This is useful for understanding partition distribution.
   */
  def getUniquePartitionValues(
    ...
    rows.map { row =>
      convertPartitionValue(row, fieldIndex, field.dataType)
    }.toSet.filter(_ != null)
  }
  
  /**
   * Create a partition path string from partition values.
  ...
  /**
   * Escape special characters in partition values for file system paths.
   * This follows Hive's path encoding rules.
   */
