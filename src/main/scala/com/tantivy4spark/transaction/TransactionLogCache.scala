...
/**
 * Time-based cache for transaction log data with configurable expiration.
 * Designed to reduce repeated CloudStorageProvider calls for read operations.
 */
class TransactionLogCache(expirationSeconds: Long = 5 * 60L) {
  ...
  /**
   * Invalidate caches that depend on the versions list
   * (called when new versions are added)
   */
  ...
  private def cleanup(): Unit = {
    ...
    val expiredVersions = versionCache.asScala.filter { case (_, entry) =>
