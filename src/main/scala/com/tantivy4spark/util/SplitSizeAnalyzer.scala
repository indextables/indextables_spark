...
class SplitSizeAnalyzer(
   ...
   */
  case class SplitInfo(size: Long, numRecords: Long, bytesPerRecord: Double)

  /**
   * Calculate target number of rows per split based on historical data.
   *
   * @param targetSizeBytes Target size in bytes for each split
   * @param maxHistoricalSplits Maximum number of recent splits to analyze (default: 10)
   * @return Target number of rows per split, or None if insufficient historical data
   */
  def calculateTargetRows(targetSizeBytes: Long, maxHistoricalSplits: Int = 10): Option[Long] = {
    try {
      ...
      try {
        ...
        logger.info(f"Auto-sizing analysis: target size=${SizeParser.formatBytes(targetSizeBytes)}, " +
  ...
  private def getRecentSplitInfo(transactionLog: TransactionLog, maxSplits: Int): Seq[SplitInfo] = {
    ...
    val validSplits = addActions.filter { action =>
      action.size > 0 && action.numRecords.exists(_ > 0)
    }.takeRight(maxSplits) // Take the most recent valid splits
    ...
    if (splitInfos.nonEmpty) {
      ...
      logger.debug(f"Historical split analysis: " +
...
object SplitSizeAnalyzer {
   ...
   */
  def apply(tablePath: Path, spark: SparkSession, options: CaseInsensitiveStringMap): SplitSizeAnalyzer = {
