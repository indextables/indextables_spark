...
/**
 * Simplified adapter that delegates to TantivyDirectInterface instances.
 * Maintains compatibility with the old handle-based API.
 */
object TantivyJavaAdapter {
  ...
  def search(indexHandle: Long, query: String, limit: Int): String = {
    try {
      Option(interfaces.get(indexHandle)) match {
        case Some(interface) =>
