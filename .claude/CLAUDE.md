...
- **TEXTSEARCH/FIELDMATCH operators**: Type-safe search (`content TEXTSEARCH 'query'` for tokenized fields, `status FIELDMATCH 'value'` for non-tokenized fields, `* TEXTSEARCH 'query'` / `* FIELDMATCH 'query'` / `* indexquery 'query'` for all fields; `indexquery`/`indexqueryall` remain the general-purpose syntax without type validation)
...
- **Text**: Full-text search via TEXTSEARCH operator (or IndexQuery). Configure: `spark.indextables.indexing.typemap.text: "field1,field2"`
...
Available commands: `MERGE SPLITS`, `PURGE INDEXTABLE`, `DROP INDEXTABLES PARTITIONS`, `CHECKPOINT/COMPACT INDEXTABLES`, `TRUNCATE INDEXTABLES TIME TRAVEL`, `PREWARM INDEXTABLES CACHE`, `WAIT FOR INDEXTABLES PREWARM JOBS`, `BUILD INDEXTABLES COMPANION`, `DESCRIBE INDEXTABLES STATE/DISK CACHE/STORAGE STATS/ENVIRONMENT/MERGE JOBS/COMPONENT SIZES/PREWARM JOBS/TRANSACTION LOG/PROFILER [CACHE]`, `FLUSH INDEXTABLES DISK CACHE/SEARCHER CACHE`, `INVALIDATE INDEXTABLES TRANSACTION LOG CACHE`, `REPAIR INDEXFILES TRANSACTION LOG`, `ENABLE/DISABLE INDEXTABLES PROFILER`, `RESET INDEXTABLES PROFILER [CACHE]`.
